exports.parseSentences = (source, target, regex) => {
    ...
    while (sourcePtr < sourceLen && targetPtr < targetLen) {
        ...
        console.log(sourcePtr + ": " + sourceToken + ", " + targetPtr + ": " + targetToken);
        console.log();
    ...
    while (sourcePtr < sourceLen) {
        deleteList.push(sourceTokens[sourcePtr]);
        sourcePtr++;
    }
    while (targetPtr < targetLen) {
        insertList.push(targetTokens[targetPtr]);
        targetPtr++;
    }
    if (deleteList.length > 0) {
        result.push({
